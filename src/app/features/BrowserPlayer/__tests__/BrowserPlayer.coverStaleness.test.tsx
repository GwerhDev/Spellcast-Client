...
// up while the SECOND spell's fetch is still in flight.
...
beforeEach(() => {
  ...
  // Minimal speechSynthesis stub -- this suite is about the cover-image effect,
  // not playback, but BrowserPlayer unconditionally touches speechSynthesis on
  // mount (voice list wiring) and, since autoPlayOnLoad is false here, never
...
describe('BrowserPlayer cover staleness across a spell switch', () => {
  it('does not keep showing the previous spell\'s cover while the new spell\'s cover is still loading', async () => {
    ...
    expect(screen.getByTestId('browser-player-cover')).toHaveAttribute('src', 'blob:mock-0');
    ...
    expect(screen.queryByTestId('browser-player-cover')).not.toBeInTheDocument();
    expect(screen.getByTestId('browser-player-cover-placeholder')).toBeInTheDocument();
    expect(screen.getByTestId('browser-player-title')).toHaveTextContent('Spell Two');
    ...
    expect(screen.getByTestId('browser-player-cover')).toHaveAttribute('src', 'blob:mock-1');
  });
});
